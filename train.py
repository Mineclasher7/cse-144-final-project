...
# -----------------------------
# Dataset — ALL 1000 images, no val split
# -----------------------------
def get_loader(train_dir, batch_size=32, num_workers=2):
    ...
    print(f"Loaded {len(dataset)} images, {len(class_to_idx)} classes")
...
# epoch 0-4:  head only (backbone fully frozen)
# epoch 5-9:  unfreeze last backbone block
# epoch 10+:  unfreeze entire backbone at low LR
# -----------------------------
...
def train_model(name, model, loader, epochs, ckpt_path, class_to_idx):
    ...
    # Separate LRs: backbone gets 10x lower than head
    backbone_attr = 'features'
    head_attr     = 'classifier' if hasattr(model, 'classifier') else 'head'
    optimizer = torch.optim.AdamW([
        {'params': getattr(model, backbone_attr).parameters(), 'lr': 3e-5},
        {'params': getattr(model, head_attr).parameters(),     'lr': 3e-4},
    ], weight_decay=0.3)
...
def main():
    ...
    loader, class_to_idx = get_loader(train_dir, batch_size=BATCH_SIZE)
    num_classes = len(class_to_idx)

    train_model("efficientnet", build_efficientnet(num_classes), loader, EPOCHS,
                "ckpt_efficientnet.pt", class_to_idx)

    train_model("swin",         build_swin(num_classes),         loader, EPOCHS,
